        ...
        try:
            ...
            if ohlcv_m15:
                ...
                if hermes_signal and hermes_signal.action in ("SELL", "BUY"):
                    ...
                    # Minimum SL distance: 20 pips = $2.00
                    MIN_SL_DIST = 2.0  # $2.00 minimum SL distance
                    MIN_RR_REQ = 1.2   # minimum acceptable R:R
...
def fmt_signal(sig, price, dxy, h, display="XAUUSD", currency="$"):
    ...
    emoji_sl = {"BUY":"🔴","SELL":"🟢"}.get(action,"🔴")
    emoji_tp = {"BUY":"🟢","SELL":"🔴"}.get(action,"🟢")
    ...
    # Fallback SL/TP when AI returns 0
    ...
    ai_parts = []
    if models_display:
        ai_parts.append(f"🤖 {models_display}")
    if voters and voters != "?":
        ai_parts.append(f"{voters} model")
    ...
    lines = [
        ...
        f"📍 Entry: {currency}{entry:.2f}" if display not in ("BBCA","BBRI","IHSG") else f"📍 Entry: Rp{entry:,.0f}",
    ]

    # SL
    sl_str = f"{emoji_sl} SL: {currency}{sl:.2f} {_sl_pips(sl)}" if display not in ("BBCA","BBRI","IHSG") else f"{emoji_sl} SL: Rp{sl:,.0f} {_sl_pips(sl)}"
    lines.append(sl_str)

    # TP levels
    for tp_val, tp_label in [(tp1,"TP1"),(tp2,"TP2"),(tp3,"TP3"),(tp4,"TP4")]:
        if tp_val and tp_val > 0:
            tp_str = f"{emoji_tp} {tp_label}: {currency}{tp_val:.2f} {_tp_pips(tp_val)}" if display not in ("BBCA","BBRI","IHSG") else f"{emoji_tp} {tp_label}: Rp{tp_val:,.0f} {_tp_pips(tp_val)}"
            lines.append(tp_str)
...
def send_to_channel(text):
    """Send to mapping channel. Returns tg_send result."""
    if MAPPING_CHANNEL_ID:
        return tg_send(text, MAPPING_CHANNEL_ID)

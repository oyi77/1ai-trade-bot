...
_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent  # bots/../.. = project root
...
from signaler import ProactiveSignaler
...
class SubscriptionBot:
    ...
    async def cmd_subscribe(
        ...
        text = (
            ...
            f"Amount: *Rp {amount:,}*\n"
            f"Valid until: `{datetime.fromtimestamp(expires_at, tz=timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}`\n\n"
            f"Reply with /confirm_{plan} to proceed, or /cancel to abort.\n\n"
            f"*Demo Mode:* Subscriptions are auto-approved for now."
        )
        ...
        text_approved = (
            ...
            f"Plan: *{plan.capitalize()}*\n"
            f"Amount: *Rp {amount:,}*\n"
            f"Expires: `{datetime.fromtimestamp(expires_at, tz=timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}`\n\n"
            f"You'll now receive proactive trading signals. 📡\n"
            f"Link your Stockity account with /link for auto-trading."
        )
        await self._reply(update, text_approved)
    ...
    async def cmd_signal(
        ...
        valid_symbols = ["CRYPTO_IDX", "BTC_IDX", "ETH_IDX", "GOLD_IDX"]
    ...
    async def handle_callback(
        ...
        if data == "plans":
            ...
            sig = await self.signaler._generator.generate("CRYPTO_IDX")
            if sig:
                text = _signal_to_text(sig)
                keyboard = None
                if sig.is_tradeable:
                    keyboard = [[InlineKeyboardButton(
                        ...
                        callback_data=f"trade_{sig.action}_CRYPTO_IDX",
                    )]]
                await self._reply(update, text, keyboard=keyboard)
            else:

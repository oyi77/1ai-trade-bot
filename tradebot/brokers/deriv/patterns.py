...
LOG = logging.getLogger("tradebot.brokers.deriv.patterns")
...
class MomenPatternAnalyzer:
    ...
    def __init__(self, target_carriers: list[int] = None,
        ...
        self.max_jaring_ticks = max_jaring_ticks or MAX_JARING_TICKS
        self.analysis_ticks = analysis_ticks
        self.min_momen1 = min_momen1
        self.min_momen2 = min_momen2

    def analyze(self, ticks: list[DerivTick]) -> MomenAnalysis | None:
        ...
        for idx, tick in enumerate(ticks[:self.analysis_ticks]):
            ...
            if idx + 1 < len(ticks):
                ...
                for j in range(2, self.max_jaring_ticks + 1):
                    if idx + j < len(ticks):  # noqa: SIM102
                        if curr_digit in self.target_carriers and ticks[idx + j].digit == 7:  # noqa: SIM102
        ...
        for carrier in self.target_carriers:
            ...
            if len(m1) >= self.min_momen1 and len(m2) >= self.min_momen2:  # noqa: SIM102
...
class AdjacencyPatternAnalyzer:
    ...
    def analyze(self, ticks: list[DerivTick]) -> AdjacencyAnalysis | None:
...
class StreakCountdownAnalyzer:
    ...
    def analyze(self, ticks: list[DerivTick]) -> StreakAnalysis | None:

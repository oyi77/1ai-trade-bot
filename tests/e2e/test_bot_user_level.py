...
def test_bot():
    ...
    if not client.is_user_authorized():
        ...
        print(f"  python3 -c \"from telethon.sync import TelegramClient; client = TelegramClient('{SESSION_NAME}', {API_ID}, '{API_HASH}'); client.start()\"")
    ...
    if messages and messages[0].buttons:
        ...
        for i, row in enumerate(msg.buttons):
            ...
            for j, btn in enumerate(row):
                btn_type = "callback" if hasattr(btn, 'data') else "url" if hasattr(btn, 'url') else "text"
    ...
    if messages:
        ...
        checks = {
            "has_direction": "call" in response or "put" in response or "buy" in response or "sell" in response,

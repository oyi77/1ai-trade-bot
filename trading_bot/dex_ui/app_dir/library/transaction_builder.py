...
async def get_token_price_from_router_2(token_addresses, router_contract=None, router_name="PancakeSwap", token_balance_before_swap=1, is_buy=True):
    ...
    try:
        ...
        async def fetch_prices_for_chunk(chunk):
            ...
            results = await aw3._run_sync(lambda: multicall.functions.aggregate3(calls).call())
            chunk_prices = {}
            ...
            total_supply_results = await aw3._run_sync(lambda: multicall.functions.aggregate3(total_supply_calls).call())
...
async def approve_token(token_contract,
                        spender_address,
                        amount,
                        wallet_address,
                        private_key,
  ...
  current_allowance = await aw3._run_sync(
      lambda: token_contract.functions.allowance(wallet_address, spender_address).call())
  if current_allowance >= amount:
    logger.info("Token already approved for the required amount or higher.")
    return True
  nonce = await aw3._run_sync(web3.eth.get_transaction_count, wallet_address, 'pending')
  ...
  try:
    txn_hash = await aw3._run_sync(web3.eth.send_raw_transaction, signed_txn.rawTransaction)
    logger.info("Token Approval success!")
    receipt = await aw3.eth_wait_for_transaction_receipt(txn_hash, timeout=120)
    if receipt.status == 1:
        ...
        return await approve_token(token_contract, spender_address, amount, wallet_address, private_key, slippage)
  except Exception as e:
...
async def fetch_token_balance(token_address, wallet_address):
    return await aw3._run_sync(lambda: get_contract(token_address).functions.balanceOf(wallet_address).call())
...
async def fetch_balances_and_prices(token_address, wallet_address):
    usdt_balance_before_swap, token_balance_before_swap, bnb_price, bnb_balance = await asyncio.gather(
        fetch_token_balance(real_usdt_address, wallet_address),
        fetch_token_balance(token_address, wallet_address),
        aw3._run_sync(get_bnb_price),
        fetch_token_balance(real_bnb_address, wallet_address),
    )
    bnb_balance = web3.from_wei(bnb_balance, 'ether') if bnb_balance else web3.from_wei(await aw3.eth_get_balance(wallet_address), 'ether')
...
async def trade_token(token_address, wallet_settings, usdt_amount, is_buy=True, slippage=0.5, expected_price=0, router_contract=None):
    ...
    try:
        ...
        best_path, best_amount_out_min = await find_best_path(router_contract, path_options, bnb_amount, token_balance_before_swap, is_buy, usdt_amount)
        ...
        approve_token_success = await approve_token(get_contract(approve_contract_address), router_address,
        ...
        swap_tx_hash = await aw3._run_sync(web3.eth.send_raw_transaction, swap_txn['rawTransaction'])
        logger.debug(f"Swap transaction sent: {swap_tx_hash}")
        txn_receipt = await aw3.eth_wait_for_transaction_receipt(swap_tx_hash)
        
        logger.debug(f"Transaction receipt: {txn_receipt}")
        if txn_receipt.status == 1:
            swap_received = await calculate_swap_received(is_buy, token_address, wallet_address, token_balance_before_swap, usdt_balance_before_swap, swap_tx_hash)
            ...
            response = await handle_failed_transaction(swap_tx_hash, txn_receipt, token_address)
    except Exception as e:
        response = await handle_exception(e, token_address)
        logger.error(f"Exception occurred during trade: {e}")
    return response

async def adjust_balance(wallet_address, private_key, bnb_balance, usdt_balance):
    ...
    if not conversion_successful:
        try:
            ...
            gas_price = await aw3._run_sync(lambda: web3.eth.gas_price)
            # Estimate gas for the approval transaction
            estimated_gas_for_approval = await aw3._run_sync(lambda: usdt_contract.functions.approve(router_address, usdt_to_wbnb_amount_raw).estimateGas({'from': wallet_address}))
            # Estimate gas for the swap transaction
            router_contract = load_router_contract()
            estimated_gas_for_swap = await aw3._run_sync(lambda: router_contract.functions.swapExactTokensForTokens(
            ...
            approve_usdt_success = await approve_token(usdt_contract, router_address, usdt_to_wbnb_amount, wallet_address, private_key, 1)
            if approve_usdt_success:
                swap_nonce = await aw3._run_sync(web3.eth.get_transaction_count, wallet_address)
                ...
                txn_hash = await aw3._run_sync(web3.eth.send_raw_transaction, signed_txn.rawTransaction)
                txn_receipt = await aw3.eth_wait_for_transaction_receipt(txn_hash)
                ...
                usdt_balance = web3.from_wei(await fetch_token_balance(real_usdt_address, wallet_address), 'ether')
                bnb_balance = web3.from_wei(await fetch_token_balance(real_bnb_address, wallet_address), 'ether')
...
async def find_best_path(router_contract, path_options, bnb_amount, token_balance_before_swap, is_buy, usdt_amount):
    simulated_results = []
    for path in path_options:
        try:
            ...
            simulation_output = (await aw3._run_sync(lambda: router_contract.functions.getAmountsOut(simulate_amount, path).call()))[-1]
...
@logger.catch
async def build_swap_transaction(router_contract, is_buy, is_bnb, bnb_amount, usdt_amount, token_balance_before_swap, min_amount_out, best_path, wallet_address, deadline, private_key):
    try:
        ...
        gas_price = await aw3._run_sync(lambda: web3.eth.generate_gas_price() or web3.eth.gas_price)
        ...
        multicall_transactions, bribe_amount = await prepare_transactions(router_contract, is_buy, is_bnb, bnb_amount, usdt_amount, min_amount_out, best_path, wallet_address, deadline, gas_price, bribe_percent, token_balance_before_swap)
        
        if bribe_amount > 0:
            bribe_txn = create_bribe_transaction(validator_address, bribe_amount, gas_price)
            multicall_transactions.append(bribe_txn)

        signed_transaction = await execute_multicall_transaction(multicall_transactions, wallet_address, gas_price, private_key, is_buy)
...
async def build_token_swap_transaction(router_contract, usdt_amount, min_amount_out, best_path, wallet_address, deadline, gas_price, bribe_percent):
    adjusted_usdt_amount = usdt_amount - int(usdt_amount * bribe_percent)
    gas_estimate = await aw3._run_sync(lambda: router_contract.functions.swapExactTokensForTokens(
    ...
    nonce = await aw3._run_sync(web3.eth.get_transaction_count, wallet_address)
    txn = await aw3._run_sync(lambda: router_contract.functions.swapExactTokensForTokens(
        adjusted_usdt_amount, 
        min_amount_out, 
        best_path, 
        wallet_address, 
...
async def build_bnb_buy_transaction(router_contract, bnb_amount, min_amount_out, best_path, wallet_address, deadline, gas_price, bribe_percent):
    ...
    gas_estimate = await aw3._run_sync(lambda: swap_func(
        min_amount_out, 
        best_path, 
        wallet_address, 
        deadline
    ...
    nonce = await aw3._run_sync(web3.eth.get_transaction_count, wallet_address)
    txn = await aw3._run_sync(lambda: swap_func(
        min_amount_out, 
        best_path, 
        wallet_address, 
        deadline
...
async def build_token_sell_transaction(router_contract, token_balance_before_swap, min_amount_out, best_path, wallet_address, deadline, gas_price, bribe_percent):
    adjusted_token_balance = token_balance_before_swap - int(token_balance_before_swap * bribe_percent)
    gas_estimate = await aw3._run_sync(lambda: router_contract.functions.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    nonce = await aw3._run_sync(web3.eth.get_transaction_count, wallet_address)
    txn = await aw3._run_sync(lambda: router_contract.functions.swapExactTokensForTokensSupportingFeeOnTransferTokens(
        adjusted_token_balance, 
        min_amount_out, 
        best_path, 
        wallet_address, 
...
async def prepare_transactions(router_contract, is_buy, is_bnb, bnb_amount, usdt_amount, min_amount_out, best_path, wallet_address, deadline, gas_price, bribe_percent, token_balance_before_swap):
    transactions = []
    bribe_amount = 0
    if is_buy and not is_bnb:
        adjusted_amount, txn = await build_token_swap_transaction(router_contract, usdt_amount, min_amount_out, best_path, wallet_address, deadline, gas_price, bribe_percent)
        transactions.append(txn)
        bribe_amount = int(adjusted_amount * bribe_percent)
    elif is_buy:
        adjusted_amount, txn = await build_bnb_buy_transaction(router_contract, bnb_amount, min_amount_out, best_path, wallet_address, deadline, gas_price, bribe_percent)
        transactions.append(txn)
        bribe_amount = int(adjusted_amount * bribe_percent)
    else:
        adjusted_amount, txn = await build_token_sell_transaction(router_contract, token_balance_before_swap, min_amount_out, best_path, wallet_address, deadline, gas_price, bribe_percent)
        transactions.append(txn)
...
async def execute_multicall_transaction(transactions, wallet_address, gas_price, private_key, is_buy):
    multicall = get_contract(os.getenv("MULTICALL_ADDRESS"), filename="multicall_router_abi")
    if not is_buy:
    ...
    try:
        multicall_nonce = await aw3._run_sync(web3.eth.get_transaction_count, wallet_address)
    ...
    try:
        gas_estimate = await aw3.eth_estimate_gas({
            'from': wallet_address, 
    ...
    if not is_buy:
        txn_hash = await aw3._run_sync(web3.eth.send_raw_transaction, signed_transaction.rawTransaction)
        await aw3.eth_wait_for_transaction_receipt(txn_hash)
        swap_txn.update({'nonce': await aw3._run_sync(web3.eth.get_transaction_count, wallet_address, 'latest')})
        signed_swap_txn = web3.eth.account.sign_transaction(swap_txn, private_key)
        signed_transaction = signed_swap_txn
    return signed_transaction

    
async def calculate_swap_received(is_buy, token_address, wallet_address, token_balance_before_swap, usdt_balance_before_swap, swap_tx_hash):
    contract_address = token_address if is_buy else usdt_address
    contract = get_contract(contract_address)
    current_balance = await aw3._run_sync(lambda: contract.functions.balanceOf(wallet_address).call())
    ...
    if swap_received <= 0:  # If swap_received is 0 or negative, calculate from swap_tx_hash
        swap_received = await calculate_from_swap_tx_hash(swap_tx_hash, is_buy, wallet_address)
...
async def calculate_from_swap_tx_hash(swap_tx_hash, is_buy, wallet_address):
    try:
        # Fetch the transaction receipt using the transaction hash
        txn_receipt = await aw3._run_sync(web3.eth.get_transaction_receipt, swap_tx_hash)
...
async def handle_failed_transaction(swap_tx_hash, txn_receipt, token_address):
    try:
        tx = await aw3._run_sync(web3.eth.get_transaction, swap_tx_hash)
        replay_tx = {'to': tx['to'], 'from': tx['from'], 'value': tx['value'], 'data': tx['input']}
        try:
            await aw3.eth_call(to=replay_tx['to'], data=replay_tx['data'], block_parameter=tx.blockNumber - 1)
        except Exception as e:
...
async def handle_exception(e, token_address):
    if isinstance(e.args[0], dict) and 'transaction' in e.args[0]:
        tx = e.args[0]['transaction']
        receipt = await aw3._run_sync(web3.eth.get_transaction_receipt, tx)

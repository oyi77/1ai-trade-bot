...
def _calculate_levels(mtf_result: dict, action: str, cfg: dict) -> dict | None:
    ...
    # ── Cap max entry distance dari current price ──
    # Supaya SELL LIMIT kagak dipasang 20 poin di atas harga
    max_entry_dist = atr_val * 1.5  # maksimal 1.5x ATR dari current price

    if action == "BUY":
        # Entry: near support + small slip, cap dari price
        raw_entry = min(price, support + slip)
        entry = max(raw_entry, price - max_entry_dist)  # jangan terlalu jauh di bawah
        # SL: relative ke entry (bukan support yang mungkin jauh)
        ...
        sl = raw_sl
        # Cap SL sesuai max_sl_pts SOP (30 pip untuk XAUUSD)
        max_sl_pips = cfg.get("max_sl_pts", 50)
        pips_sl = abs(entry - sl) / cfg["pip_value"]
        if pips_sl > max_sl_pips:
            # Tighten SL ke max yang diizinkan
            sl = entry + (max_sl_pips * cfg["pip_value"]) if action == "SELL" else entry - (max_sl_pips * cfg["pip_value"])
        # TP based on RR
        pips_sl = abs(entry - sl) / cfg["pip_value"]
        ...
        raw_entry = max(price, resistance - slip)
        entry = min(raw_entry, price + max_entry_dist)  # jangan terlalu jauh di atas
        # SL: relative ke entry (bukan resistance yang mungkin jauh)
        # Pakai max(ATR buffer, min SL) supaya realistis
        sl_buffer = max(atr_val * cfg["sl_buffer_atr"], cfg["min_sl_pts"] * cfg["pip_value"])
        raw_sl = entry + sl_buffer
        ...
        if pips_sl > max_sl_pips:
            sl = entry + (max_sl_pips * cfg["pip_value"])  # Tighten SL
        pips_sl = abs(sl - entry) / cfg["pip_value"]
        tp1_price = entry - (pips_sl * cfg["pip_value"])       # 1:1
        tp2_price = entry - (pips_sl * 2 * cfg["pip_value"])   # 1:2

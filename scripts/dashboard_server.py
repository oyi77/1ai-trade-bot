...
function renderSignals(signals) {
    ...
    el.innerHTML = signals.slice().reverse().map(s => {
        ...
        const source = s.source === 'user-generate' 
            ? `👤 Generated by @${s.source_user || 'anon'}`
            : '🤖 Vilona AI Auto-Scanner';
        const outcome = s.status !== 'pending' 
            ? `<span style="color:${s.status==='tp'?'#00d4aa':'#ff4757'}">${s.status.toUpperCase()}</span>`
            : '<span style="color:#888">PENDING</span>';
        
        return `<div class="${cls}">
            <div class="sig-header">
                <span class="${actionCls}">${emoji} ${s.direction} ${s.symbol}</span>
                <span class="sig-symbol">${outcome} · ${ts}</span>
            </div>
            ...
            <div class="sig-engines">
                ${engineKeys.length ? '<div class="engines-grid">' + engineKeys.map(k => 
                    `<span>${k}: ${typeof engines[k]==='object' ? (engines[k].confidence*100||0).toFixed(0)+'%' : engines[k]}</span>`
                ).join('') + '</div>' : ''}
            </div>
            <div class="sig-meta">
                RR 1:${s.rr_ratio || '?'} · Conf: ${((s.confidence||0)*100).toFixed(0)}% · Grade: ${s.grade || 'N/A'}
            </div>
...
function renderUserActivity(signals) {
    ...
    el.innerHTML = signals.slice().reverse().slice(0, 10).map(s => {
        ...
        return `<div class="activity-item">
            <span class="user">@${s.source_user || 'anon'}</span>
            <span class="action" style="color:${isBuy?'#00d4aa':'#ff4757'}">${emoji} ${s.direction}</span>
            <span style="color:#888">${s.symbol}</span>
            <span class="time">${(s.timestamp||'').slice(11,16)}</span>
        </div>`;
    }).join('');
}

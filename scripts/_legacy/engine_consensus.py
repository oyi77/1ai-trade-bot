...
import logging, time, os, sys as _sys, json as _json
from typing import Any
from datetime import datetime, timezone

# Pastiin scripts/ di sys.path biar engine loader nemu modul
...
def _load_engines():
    ...
    try:
        ...
        pass
    try:
        from deriv.derive_engine import DeriveEngine
        _derive_engine = DeriveEngine()
    except Exception:
        pass
...
def run_engine_consensus(
    ...
    # ── Deriv Engine (standalone — runs once regardless of TF) ──
    deriv_result = None
    if _derive_engine:
        try:
            deriv_raw = _derive_engine.analyze(symbol=symbol)
            if deriv_raw and deriv_raw.get("direction") != "ERROR":
                deriv_result = deriv_raw
                logger.info(
                    "DerivEngine: %s %s (conf=%.1f%%)",
                    symbol, deriv_raw["direction"], deriv_raw["confidence"] * 100,
                )
        except Exception as e:
            logger.warning("DerivEngine error: %s", e)
